...
public final class BlinkLocalService extends BlinkLocalBaseService {
	...
	public String receiveMessageFromProcessor(String message) {
		...
		// BlinkApp 동기화일 경우
			...
			List<BlinkAppInfo> mBlinkAppInfoList = gson.fromJson(
					mDatabaseMessage.getData(),
					new TypeToken<List<BlinkAppInfo>>() {
					}.getType());
			// 자기 자신이 메인 디바이스일 경우
			if (BlinkDevice.HOST.getAddress().contentEquals(
					mServiceKeeper.obtainCurrentCenterDevice().getAddress())) {
				return ""
						+ mSyncDatabaseManager.center
								.syncBlinkDatabase(mBlinkAppInfoList);
			}
			// 메인 디바이스가 아닐 경우
			else {
				return ""
						+ mSyncDatabaseManager.wearable
								.syncBlinkDatabase(mBlinkAppInfoList);
			}
		}
		// MeasurementData 저장일 경우
		else if (mDatabaseMessage.getType() == DatabaseMessage.SYNC_MEASUREMENT) {
			List<MeasurementData> mMeasurementDataList = gson.fromJson(
					mDatabaseMessage.getData(),
					new TypeToken<List<MeasurementData>>() {
					}.getType());
			mSyncDatabaseManager.center
					.insertMeasurementData(mMeasurementDataList);
			return "true";
		}
		return null;
	}
		...
		public void onChange(boolean selfChange, Uri uri) {
			...
			if (uri.equals(SqliteManager.URI_OBSERVER_BLINKAPP)) {
				...
				BlinkMessage mBlinkMessage = new BlinkMessage.Builder()
						...
						.setMessage(gson.toJson(mDatabaseMessage)).setCode(0)
						.build();
