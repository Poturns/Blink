...
public class MessageProcessor {
	...
	public void acceptBlinkMessage(BlinkMessage blinkMessage,
		...
		if (blinkMessage.getDestinationAddress().equals(currentAddress)) {// Message의
			...
			if (blinkMessage_type == IBlinkMessagable.TYPE_REQUEST_MEASUREMENTDATA_SYNC) {
			...
			} else if (blinkMessage_type == IBlinkMessagable.TYPE_REQUEST_NETWORK_SYNC) { // center 보내주던간에 
				...
				ServiceKeeper
						...
						.transferSystemSync(device,
								IBlinkMessagable.TYPE_REQUEST_BlinkAppInfo_SYNC); // 여기가 문제야.
				// BlinkMessage successBlinkMessage = builder_success.build();
			...
			} else { // 해당 Device와 연결되지 않아서 Pass가 불가능할 때 FAIL Message를 보낸
						// 디바이스쪽으로 보내준다.
						// 차후 : 현재는 그룹 간 통신을 고려하지 않아 Source Device가 Main에서 한 단계만
						// 떨어져있다는 가정하에 보내준다. 만약
						// 그룹별 연결되어있을 시에는 해당 Source로 향하는 next hop을 검색해서
						// BlinkDevice를 설정한다던지의 처리가 필요함.
				BlinkMessage.Builder builder = new BlinkMessage.Builder(); // Set
																			// the
																			// type_response
																			// on
																			// type.
	 ...
	 * BlinkDevice targetBlinkDevice =
	 * BlinkDevice.load(blinkMessage.getDestinationAddress());
	 * if(targetBlinkDevice.isConnected()){ // 타겟 디바이스와 연결되어있을 경우 해당 device로
	 * pass sendBlinkMessageTo(blinkMessage, targetBlinkDevice); } else{ // 타겟
	 * 디바이스와 연결되어 있지 않은 경우 요청한 디바이스에 fail message를 보낸다.
	 * 
	 * //sendBlinkMessageTo(,fromDevice); } }/* ======= String ret =
	 * OPERATOR_CONTEXT.receiveMessageFromProcessor(message.getMessage());
	 * SERVICE_KEEPER
	 * .obtainBinder(message.getSourceApplication()).callbackData(message
	 * .getCode(), ret);
	 * 
	 ...
	 * .setDestinationDevice(null) .setDestinationApplication(null)
	 * .setReliable(false) .setType(BlinkMessage.TYPE_REQUEST_FUNCTION)
	 * .setMessage("{JSON : ...}") .build(); sendBlinkMessageTo(msg,
	 * BlinkDevice.load( msg.getDestinationAddress() ));
	 */
	...
	public void sendBlinkMessageTo(BlinkMessage message, BlinkDevice toDevice) {
		 ...
		 * 
		 * 2.1 Destination Mac != null -> Hop:Main, Node:Wearable (w-m-w) 2.2
		 * Destination Mac != null -> Hop:Wearble, Node:Wearble(w-w)
		 * 
		 * 
		...
		if (SERVICE_KEEPER.obtainCurrentCenterDevice() != BlinkDevice.HOST) {
			...
			// center일 때도 2가지로 나눠 1. Accept에서 send할 때. 2 send할 때 -> Accept에서
			// 걸러야함.
