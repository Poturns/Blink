...
/**
 * 자신의 디바이스를 분석하여, {@link Identity}를 도출하는 모듈. <br>
 * Identity 관련 계산을 수행하는 메소드들을 제공한다.
 * 
 * @author Yeonho.Kim
 * @since 2014.07.17
 */
public class DeviceAnalyzer {
	 ...
	 * 
	 * <p>
	 * {@link #UNKNOWN} : 디바이스에 대한 Identity를 파악할 수 없거나, Blink 기능을 수행하기에 부적합. <br>
	 * {@link #PERIHERAL} : 주변부 디바이스. PROXY는 가능하지만, MAIN의 역할을 수행할 수 없다. <br>
	 * {@link #CORE} : 일반 디바이스. PROXY & MAIN의 역할을 모두 수행할 수 있다. <br>
	 * {@link #PROXY} : 현재 네트워크 그룹에서 임시적으로 중심부 역할을 대리한다. <br>
	 * {@link #MAIN} : 현재 네트워크 그룹에서 중심부 역할을 담당한다.
	 * 
	 * @author Yeonho.Kim
	 *
	 */
	public enum Identity {
		...
		/**
		 * <b>[ CENTER ]</b> <br>
		 * : 네트워크 그룹에서의 중심부. 연결된 각 디바이스간의 데이터를 중개하고, 연결해준다.
		 * 
		 * <p>
		 */
		PROXY,
	...
	private boolean getMainIdentityFromPref(){
		...
		boolean isMain = pref.getBoolean(ANALYZER_CONTEXT.getString(R.string.res_blink_preference_external_key_set_this_device_to_main), false);
	 ...
	 */
	int analyze() {
		ConnectivityManager mConnManager = (ConnectivityManager) ANALYZER_CONTEXT
	 ...
	 * <p>
	 * ※ Identity.CORE 이상부터 MAIN Identity가 될 수 있다.
	 * 
	 * @param enable
	 */
	synchronized boolean grantMainIdentityFromUser(boolean enable) {
		...
		if (BlinkDevice.HOST != null) {
			...
			mIdentityPoint = enable ? (mIdentityPoint | IDENTITY_POINTLINE_USER)
	...
	synchronized boolean grantMainIdentity(boolean enable) {
		if (BlinkDevice.HOST != null) {
			...
			if (mIdentityPoint >= IDENTITY_POINTLINE_CORE) {
				mIdentityPoint = enable ? (mIdentityPoint | IDENTITY_POINTLINE_MAIN
						^ IDENTITY_POINTLINE_PROXY)
	 ...
	 * <p>
	 * ※ Identity.MAIN은 Proxy Identity가 될 수 없다.
	 * 
	 * @param enable
	 * @return
	 */
	synchronized boolean grantProxyIdentity(boolean enable) {
		if (BlinkDevice.HOST != null) {
			...
			mIdentityPoint = enable ? (mIdentityPoint | IDENTITY_POINTLINE_PROXY)
	...
	/**
	 * 결과값은 (device1 - device2)에 해당하는 차이 값이다. 따라서 결과값이 양수일 경우 device1이 더 크고, 음수일
	 * 경우 device2가 더 큰 경우이다.
	 * 
	 * @param device1
	 * @param device2
	 * @return
	...
	long compareForIdentity(BlinkDevice device1, BlinkDevice device2) {
		int mIdentityPointDev1 = device1.getIdentityPoint()
				^ IDENTITY_POINTLINE_PROXY;
		int mIdentityPointDev2 = device2.getIdentityPoint()
				^ IDENTITY_POINTLINE_PROXY;
		...
		if (mIdentityPointDev1 != mIdentityPointDev2) {
			...
			return mTimestampDev1 - mTimestampDev2;
		}
	}
	...
	public void broadcastIdentityChanged(BlinkDevice device, Identity identity) {
		Intent intent = new Intent(
				IBlinkEventBroadcast.BROADCAST_DEVICE_IDENTITY_CHANGED);
		intent.putExtra(IBlinkEventBroadcast.EXTRA_DEVICE,
				(Serializable) device);
	...
	public boolean isAvailableInternet() {
		int internetFlag = IDENTITY_POINTLINE_WIFI | IDENTITY_POINTLINE_MOBILE
				| IDENTITY_POINTLINE_ETHERNET;
	...
	/**
	 * 현 디바이스가 해당 Feature를 지원하는지 여부를 반환한다. 해당 Feature에 해당하는 IDENTITY_POINTLINE을
	 * 매개변수로 전달한다.
	 *
	...
	String generateGroupId() {
		...
		String strID = BlinkDevice.HOST.getAddress() + "@"
				+ System.currentTimeMillis();
		return EncryptionUtil.grantHashMessage(strID.toUpperCase()).substring(
				1, 40);
	}
