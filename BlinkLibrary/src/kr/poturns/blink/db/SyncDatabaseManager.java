...
public class SyncDatabaseManager extends BlinkDatabaseManager {
	...
	private HashMap<Integer, Integer> obtainMeasurementMap(
			List<BlinkAppInfo> SystemDatabaseObjctList) {
		...
		for (int i = 0; i < newSystemDatabaseObject.size(); i++) {
			...
			for (int j = 0; j < oldSystemDatabaseObject.size(); j++) {
				...
				if (newSystemDatabaseObject.get(i).mDevice.MacAddress
						.contentEquals(oldSystemDatabaseObject.get(j).mDevice.MacAddress)
						&& newSystemDatabaseObject.get(i).mApp.PackageName
								.contentEquals(oldSystemDatabaseObject.get(j).mApp.PackageName)) {
					...
					for (int k = 0; k < newMeasurementList.size(); k++) {
						...
						for (int l = 0; l < oldMeasurementList.size(); l++) {
							...
							if (newMeasurement.Measurement
									.contentEquals(oldMeasurement.Measurement)) {
								if (oldMeasurement.MeasurementId != newMeasurement.MeasurementId)
									MeasurementMap.put(
											oldMeasurement.MeasurementId,
	 ...
	 * @return
	 */
	private List<BlinkAppInfo> obtainAddedBlinkAppInfo(
			List<BlinkAppInfo> BlinkAppInfoList) {
		...
		for (int i = 0; i < WearableBlinkAppInfoList.size(); i++) {
			...
			for (int j = 0; j < MainBlinkAppInfoList.size(); j++) {
				// 동일한 systemDatabaseObject 검색
				if (WearableBlinkAppInfoList.get(i).mDevice.MacAddress
						.contentEquals(MainBlinkAppInfoList.get(j).mDevice.MacAddress)
						&& WearableBlinkAppInfoList.get(i).mApp.PackageName
								.contentEquals(MainBlinkAppInfoList.get(j).mApp.PackageName)
						&& WearableBlinkAppInfoList.get(i).mApp.Version == MainBlinkAppInfoList
								.get(j).mApp.Version) {
					// 동일한 SystemDatabaseObject가 존재하면 추가된게 아니기 때문에 isAdded 값
					// false로 변경
					isAdded = false;
				}
			}
			...
			if (isAdded)
				AddedBlinkAppInfoList.add(WearableBlinkAppInfoList.get(i));
		}
	...
	/**
	 * BlinkAppInfo에 있는 정보들을 모두 입력한다. <br>
	...
	/**
	 * BlinkAppInfo에 있는 Device 정보를 입력한다.<br>
	 * ID, DateTime도 그대로 입력된다. insertBlinkApp()에서 호출된다.
	 * 
	 * @param mBlinkAppInfo
	 */
	...
	/**
	 * BlinkAppInfo에 있는 App 정보를 입력한다. <br>
	 * ID, DateTime도 그대로 입력된다. insertBlinkApp()에서 호출된다.
	 * 
	 * @param mBlinkAppInfo
	 */
	...
	/**
	 * BlinkAppInfo에 있는 Measurement 정보를 입력한다.<br>
	 * ID, DateTime도 그대로 입력된다. insertBlinkApp()에서 호출된다.
	 * 
	 * @param mBlinkAppInfo
	 */
	...
	/**
	 * BlinkAppInfo에 있는Function 정보를 입력한다.<br>
	 * ID, DateTime도 그대로 입력된다. insertBlinkApp()에서 호출된다.
	 * 
	 * @param mBlinkAppInfo
	 */
	...
	/**
	 * SyncMeasurementData를 DB에 저장한다. <br>
	 * SyncMeasurementData가 기존에 저장되어 있지 않을 경우에 호출된다.
	 * 
	 * @param mSyncMeasurementData
	 */
	private void registerSyncMeasurementData(
			SyncMeasurementData mSyncMeasurementData) {
		ContentValues values = new ContentValues();
	...
	/**
	 * SyncMeasurementData를 변경한다. SyncMeasurementData가 기존에 저장되어 있을 경우에 호출된다.
	 * 
	 * @param mSyncMeasurementData
	 */
	...
	/**
	 * MeasurementData 테이블의 MeasurementDataId 칼럼에서 가장 큰 값을 찾아준다. 만약 없으면 0을 리턴한다.
	...
	/**
	 * SystemDatabase 테이블을 모두 삭제한다.<br>
	 * Device,App,Function,Measurement 테이블을 삭제한다. wearable 디바이스에서 center로부터 받은
	 * BlinkAppInfo를 입력하기 전에 호출된다.
	 */
		...
		while (mapSet.size() > 0) {
			...
			if (mapIterator.hasNext()) {
				...
				if (MeasurementMap.get(value) != null) {
					...
					while (MeasurementMap.get(tempMeasurementId) != null)
						tempMeasurementId++;
	...
	/**
	 * MeausrmentData 테이블을 업데이트하는 매소드 <br>
	 * Center로부터 받은 BlinkAppInfo에서 MeasurementId 변경 될 값을 확인 후 해당 데이터들의 Id를
	 * 업데이트한다.
	 * 
	 * @param set
	 * @param where
	...
	public class Wearable {
		...
		/**
		 * 해당 디바이스에 보낼 MeasurementData를 얻어온다. 기존에 보냈던 데이터가 있으면 이후 데이터를 보낸다.
		 */
		public List<MeasurementData> obtainMeasurementDatabase(
				BlinkDevice mBlinkDevice) {
			clear();
			queryDevice("MacAddress='" + mBlinkDevice.getAddress() + "'");
			if (getDeviceList().size() == 0)
				return null;
			queryMeasurementData("MeasurementDataId > "
					+ obtainSequence(getDeviceList().get(0).DeviceId));
			return getMeasurementDataList();
		}

		/**
		 * syncMeasurementData 테이블을 업데이트한다. <br>
	...
	public class Center {
		 ...
		 * @return
		 */
		public int insertMeasurementData(
